## This file is based on 20240614_Asano_motif_1_34to51.txt
...
# RPKMtable$CDS_18bp <- as.character(NA)
# RPKMtable$CDS_22to51 <- as.character(NA)
# RPKMtable$CDS_28to51 <- as.character(NA)
# RPKMtable$CDS_16to51 <- as.character(NA)
RPKMtable$CDS_34to51 <- as.character(NA)
for(i in 1:nrow(RPKMtable)){
	...
	if(length(index) == 1){
		...
		# RPKMtable$CDS_18bp[i] <- as.character(ORFs[[index]][1:18])
		# RPKMtable$CDS_18bp[i] <- as.character(ORFs[[index]][1:12])
		# RPKMtable$CDS_22to51[i] <- as.character(ORFs[[index]][22:51])
		# RPKMtable$CDS_28to51[i] <- as.character(ORFs[[index]][28:51])
		# RPKMtable$CDS_16to51[i] <- as.character(ORFs[[index]][16:51])
		RPKMtable$CDS_34to51[i] <- as.character(ORFs[[index]][34:51])
	}
}

length(which(is.na(RPKMtable$CDS_34to51) == TRUE))
...
# > head(AsanoDF, n = 3)
#      Gene         CDS_34to51
# 1 YJL153C GTTAAAGTAGTTACCGAC
# 2 YOL154W GCAACTATTGCTTCTCTA
# 3 YDR206W CCTACCGCTTTTAATGGT
...
for(i in 1:length(matchList)){
	...
	matchLines[(i-1)*2+1] <- AsanoDF$CDS_34to51[i]
...
setwd(HOMEDIR)
setwd("XLSX")
write.xlsx(compareDF, file = "20240614_AGYYGRY_6thTrial_4940_34to51.xlsx")
...
# > colnames(matchDF)
#  [1] "Gene"      "CDS_34to51"  "AGYYGNN"   "NGYYGRN"   "NNYYGRY"   "NGYYGRY"  
...
matchDF$hit[is.na(matchDF$CDS_34to51) == TRUE] <- NA


setwd(HOMEDIR)
setwd("XLSX")
write.xlsx(matchDF, file = "20240614_matchDF_6thTrial_4940_34to51.xlsx")
