## This file is based on 20240614_Asano_motif_1_12nt.txt
...
RPKMtable$CDS_12bp <- as.character(NA)	
for(i in 1:nrow(RPKMtable)){
	...
	if(length(index) == 1){
		...
		RPKMtable$CDS_12bp[i] <- as.character(ORFs[[index]][1:12])
	}
}

length(which(is.na(RPKMtable$CDS_12bp) == TRUE))
...
# > head(AsanoDF, n = 3)
#      Gene     CDS_12bp
# 1 YJL153C ATGACAGAAGAT
# 2 YOL154W ATGAAGTTCTCT
# 3 YDR206W ATGGAACCATCG
...
for(i in 1:length(matchList)){
	...
	matchLines[(i-1)*2+1] <- AsanoDF$CDS_12bp[i]
...
setwd(HOMEDIR)
setwd("XLSX")
write.xlsx(compareDF, file = "20240614_AGYYGRY_6thTrial_4940_12bp.xlsx")
...
# > colnames(matchDF)
#  [1] "Gene"      "CDS_12bp"  "AGYYGNN"   "NGYYGRN"   "NNYYGRY"   "NGYYGRY"  
...
matchDF$hit[is.na(matchDF$CDS_12bp) == TRUE] <- NA


setwd(HOMEDIR)
setwd("XLSX")
write.xlsx(matchDF, file = "20240614_matchDF_6thTrial_4940_12bp.xlsx")
