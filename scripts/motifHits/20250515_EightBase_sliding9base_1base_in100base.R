## original note: 	20250515_EightBase_sliding9base_1base_in100base.R.txt
## 			20250515_EightBase_sliding12base_1base_in100base.R.txt
## 			20250515_EightBase_sliding15base_1base_in100base.R.txt
## 			20250515_EightBase_sliding18base_1base_in100base.R.txt
## 			20250515_EightBase_sliding21base_1base_in100base.R.txt
## 			20250515_EightBase_sliding24base_1base_in100base.R.txt
## 			20250515_EightBase_sliding27base_1base_in100base.R.txt
...
setwd("motifHits")
setwd("XLSX")
EightBase_hits <- read.xlsx(xlsxFile = "20250515_EightBase_hits_100base.xlsx", colNames = TRUE)
...
STARTsEND <- 100 - slideLEN + 1	
ENDsSTART <- slideLEN
STARTs <- seq(1, STARTsEND, baseNum)
ENDs <- seq(ENDsSTART, 100, baseNum)	
...
out3 <- data.frame(start = STARTs, end = ENDs, intercept = as.numeric(NA), slope = as.numeric(NA))
...
fileName <- paste(DAY, "_EightBase_slope_LEN", slideLEN, "_", baseNum, "base_in100base.pdf", sep = "")
pdf(file = fileName)

p <- ggplot(out3, aes(start, -slope)) + geom_smooth(method = "loess", span = 0.2)
p + geom_point()
...
DAY <- gsub(pattern = "-", replacement = "", x = as.character(Sys.Date()))
fileName2 <- paste(DAY, "_EightBase_hits_LEN", slideLEN, "_", baseNum, "base_in100base.pdf", sep = "")
pdf(file = fileName2)
...
for(i in 1:length(STARTs)){
	...
	x = EightBase_hits_per51$rank
	y = EightBase_hits_per51[[colName]]
	a = out3$intercept[out3$end == END]
	b = out3$slope[out3$end == END]
...
outList <- list(EightBase_hits = EightBase_hits, 
		...
		bases_x_to_y = out3)
DAY <- gsub(pattern = "-", replacement = "", x = as.character(Sys.Date()))
fileName3 <- paste(DAY, "_EightBase_slope_LEN", slideLEN, "_", baseNum, "base_in100base.xlsx", sep = "")
