## original note: 	20250515_EightBase_sliding9base_1base.R.txt
## 			20250515_EightBase_sliding12base_1base.R.txt
## 			20250515_EightBase_sliding15base_1base.R.txt
## 			20250515_EightBase_sliding18base_1base.R.txt
## 			20250515_EightBase_sliding21base_1base.R.txt
## 			20250515_EightBase_sliding24base_1base.R.txt
## 			20250515_EightBase_sliding27base_1base.R.txt
...
setwd("motifHits")
setwd("XLSX")
EightBase_hits <- read.xlsx(xlsxFile = "20250513_EightBase_hits.xlsx", colNames = TRUE)
...
EightBase_hits <- cbind(data.frame(geneName = geneNames, sysName = sysNames, rank = 1:246), 
			EightBase_hits)

EightBase_hits <- EightBase_hits[, c(4, 2, 3, 5, 6)]

STARTsEND <- 51 - slideLEN + 1
ENDsSTART <- slideLEN
STARTs <- seq(1, STARTsEND, baseNum)
ENDs <- seq(ENDsSTART, 51, baseNum)
...
fileName <- paste(DAY, "_EightBase_slope_LEN", slideLEN, "_", baseNum, "base.pdf", sep = "")
pdf(file = fileName)

p <- ggplot(out3, aes(start, -slope)) + geom_smooth(method = "loess")
p + geom_point()
...
DAY <- gsub(pattern = "-", replacement = "", x = as.character(Sys.Date()))
fileName2 <- paste(DAY, "_EightBase_hits_LEN", slideLEN, "_", baseNum, "base.pdf", sep = "")
pdf(file = fileName2)
...
outList <- list(EightBase_hits = EightBase_hits, 
		...
		bases_x_to_y = out3)
DAY <- gsub(pattern = "-", replacement = "", x = as.character(Sys.Date()))
fileName3 <- paste(DAY, "_EightBase_slope_LEN", slideLEN, "_", baseNum, "base.xlsx", sep = "")
