## original note: 20250515_Asano_motif_1.R.txt
...
for(i in 1:nrow(AsanoDF)){
	...
	if(length(index) == 1){
		...
		if(length(CDS) > 100) AsanoDF$CDSover100[i] <- TRUE
	}
}
...
if(length(YDR278C_CDS) > 100) AsanoDF$CDSover100[i] <- TRUE
...
AsanoDF[AsanoDF$CDSover100 == FALSE, c(1:6, 8, 9)]

AsanoDF2 <- AsanoDF[AsanoDF$CDSover100 == TRUE,]

AsanoDF2$CDS_100bp <- as.character(NA)
for(i in 1:nrow(AsanoDF2)){
	CDS <- AsanoDF2$orf_coding[i]
	AsanoDF2$CDS_100bp[i] <- substr(x = CDS, start = 1, stop = 100)
}
...
for(i in 1:nrow(AsanoDF2)){
	...
	matchList[[GENE]] <- integer(length = 100)
	SEQ <- AsanoDF2$CDS_100bp[i]
...
for(i in 1:length(matchList)){
	...
	matchLines[(i-1)*2+1] <- AsanoDF2$CDS_100bp[i]
...
setwd("motifHits")
setwd("XLSX")
write.xlsx(compareDF, file = "20250515_EightBase_100base.xlsx")
...
setwd("motifHits")
setwd("XLSX")
write.xlsx(matchDF, file = "20250515_matchDF_EightBase_100base.xlsx")
...
setwd("motifHits")
setwd("XLSX")
write.xlsx(outList, file = "20250515_motifEnrichment_EightBase_100base.xlsx")
...
setwd("motifHits")
setwd("XLSX")
write.xlsx(outList2, file = "20250515_motifEnrichment_EightBase_p0.05_topHits2_100base.xlsx")
...
setwd("motifHits")
setwd("XLSX")
inDF <- read.xlsx(xlsxFile = "20250515_EightBase_100base.xlsx")

Index <- seq(2, 492, 2)
outDF <- inDF[Index, ]

write.xlsx(outDF, file = "20250515_EightBase_hits_100base.xlsx")
